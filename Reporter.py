...
class Reporter:
	...
	def report(self, meanObjective, bestObjective, bestSolution):
		if (time.perf_counter() - self.startTime < self.allowedTime + self.writingTime):
			...
			self.writingTime += time.perf_counter() - start
		return (self.allowedTime + self.writingTime) - (time.perf_counter() - self.startTime)
